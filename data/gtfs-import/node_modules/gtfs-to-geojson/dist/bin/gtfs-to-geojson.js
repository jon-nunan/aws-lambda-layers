...
var truncateGeoJSONDecimals = (geojson, config) => {
  featureEach(geojson, (feature2) => {
    if (feature2.geometry.coordinates) {
      ...
      } else if (feature2.geometry.type.toLowerCase() === "multilinestring") {
        feature2.geometry.coordinates = feature2.geometry.coordinates.map(
          (linestring) => linestring.map((coordinate) => truncateCoordinate(coordinate, config.coordinatePrecision))
        );
...
var setupImport = async () => {
  ...
  await gtfs_to_geojson_default(config);
  process.exit();
};
setupImport().catch(handleError);
//# sourceMappingURL=gtfs-to-geojson.js.map